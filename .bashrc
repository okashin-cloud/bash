...
__conda_setup="$('/homes/sokazaki/anaconda3/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/homes/sokazaki/anaconda3/etc/profile.d/conda.sh" ]; then
        . "/homes/sokazaki/anaconda3/etc/profile.d/conda.sh"
    else
        export PATH="/homes/sokazaki/anaconda3/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<
